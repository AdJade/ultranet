...
func NewBitcoinManager(_db *badger.DB, _params *UltranetParams,
	_timeSource chainlib.MedianTimeSource, _btcDataDir string,
	_updateChan chan *ServerMessage) (
...
func (bm *BitcoinManager) writeMessage(conn net.Conn, msg wire.Message, params *UltranetParams) error {
...
func (bm *BitcoinManager) readMessage(conn net.Conn, params *UltranetParams) (wire.Message, []byte, error) {
...
func (bm *BitcoinManager) readRemoteVersionMsg(conn net.Conn, params *UltranetParams) (_minVersion uint32, _err error) {
...
// remote peer.
func localVersionMsg(conn net.Conn, tipHeight int32, params *UltranetParams) (*wire.MsgVersion, error) {
...
func _connectToPeer(addrMgr *addrmgr.AddrManager, dialTimeout time.Duration) (net.Conn, *wire.NetAddress) {
...
func (bm *BitcoinManager) _negotiateVersion(conn net.Conn, height int32, params *UltranetParams) error {
...
func _findPrevTestNetDifficulty(startNode *BlockNode, params *UltranetParams) *BlockHash {
...
func _calcNextRequiredDifficulty(lastNode *BlockNode, newBlockTime time.Time, params *UltranetParams) (*BlockHash, error) {
	// Genesis block.
	if lastNode == nil {
	...
	// difficulty.
	actualTimespan := lastNode.Header.TstampSecs - firstNode.Header.TstampSecs
...
func (bm *BitcoinManager) ProcessBitcoinHeaderQuick(bitcoinHeader *wire.BlockHeader, params *UltranetParams) (
	...
	if *parentNode.Hash != *bm._headerTip().Hash {
		return false, false, fmt.Errorf("BitcoinManager.ProcessBitcoinHeaderQuick: "+
			"Processing header %v with parent %v that is not equal to the current "+
			"tip %v; this should never happen", bitcoinHeader, parentNode, bm._headerTip())
	}
	...
	newNode := NewBlockNode(
		...
		big.NewInt(0),
		// We are bastardizing the Ultranet header to store Bitcoin information here. However,
		// it is important to note that they are similar enough such that if one were to
		// take the MsgUltranetHeader information stored here and convert it to a wire.BlockHeader,
		...
		&MsgUltranetHeader{
			...
			TstampSecs:            uint32(bitcoinHeader.Timestamp.Unix()),
			Height:                parentNode.Height + 1,
			Nonce:                 bitcoinHeader.Nonce,
		},
		StatusNone,
	)
...
func (bm *BitcoinManager) _computePow(tstamp time.Time, headerHash *BlockHash,
	parentNode *BlockNode, params *UltranetParams, optionalDifficultyBits uint32) (
...
func (bm *BitcoinManager) ProcessBitcoinHeaderFull(bitcoinHeader *wire.BlockHeader, params *UltranetParams) (
	_isMainChain bool, _isOrphan bool, _err error) {
	bm.BitcoinHeaderIndexLock.Lock()
	defer bm.BitcoinHeaderIndexLock.Unlock()
	...
	// haven't seen before.
	_, nodeExists := bm.headerIndex[headerHash]
	if nodeExists {
...
func (bm *BitcoinManager) GetBitcoinBurnWorkBlocks(blockHeight uint32) int64 {
	// If we don't have an oldest time-current block it means we're not time-current.
	// Return zero in this case.
	oldestTimeCurrentBlock := bm.GetOldestTimeCurrentBlock()
	if oldestTimeCurrentBlock == nil {
		return 0
	}

	return int64(oldestTimeCurrentBlock.Height) - int64(blockHeight)
}

// GetOldestTimeCurrentBlock returns the oldest time-current Bitcoin
// block, which is the block furthest from the tip whose
// timestamp is below BitcoinMaxTipAge. This makes it the highest-height
// block that a time-current node is guaranteed to have. If no block
// in the main chain is younger than BitcoinMaxTipAge, which is the same as
// saying the tip is not time-current, then nil is returned.
//
// This block is used to determine the amount of work that has been
// done on a Bitcoin block in order to determine if a particular BitcoinExchange
// transaction is valid. See comment on BitcoinMinBurnWork in the parameter
// definitions for more information.
func (bm *BitcoinManager) GetOldestTimeCurrentBlock() *BlockNode {
	bm.BitcoinHeaderIndexLock.RLock()
	defer bm.BitcoinHeaderIndexLock.RUnlock()

	// As a minor optimization, if we have a lot of nodes in our best header chain,
	// start at an offset from the tip that points to a node we are pretty sure is not
	// current.
	startIndex := 0
	maxTipAgeBlocks := int64(bm.params.BitcoinMaxTipAge) / int64(bm.params.BitcoinTimeBetweenBlocks)
	tipOffset := int(maxTipAgeBlocks * 10)
	if len(bm.bestHeaderChain) > tipOffset {
		startIndex = len(bm.bestHeaderChain) - tipOffset
	}

	// Iterate through the best header chain and return the first node we find that is
	// current. This will be a node that nearly all nodes that thing they're up-to-date
	// will have.
	for _, node := range bm.bestHeaderChain[startIndex:] {
		if bm._isCurrentNode(node, false /*considerCumWork*/) {
			return node
		}
	}

	// If we get here then we didn't find any nodes that are time-current so return nil.
	return nil
}
...
func (bm *BitcoinManager) _isCurrentNode(node *BlockNode, considerCumWork bool) bool {
	...
	if tipTime.Before(oldestAllowedTipTime) {
		//glog.Tracef("BitcoinManager.isCurrent: Tip time %v is before oldest "+
		//"allowed time %v", tipTime, oldestAllowedTipTime)
		return false
	}
...
func (bm *BitcoinManager) __sendGetHeaders(conn net.Conn) {
	...
	// and the tip of our best Bitcoin header chain if the tip
	// differs from the start node.
	headerHashes := []*BlockHash{bm.params.BitcoinStartBlockNode.Hash}
	if len(bm.bestHeaderChain) > 1 {
		// Only add the tip if it's different from the start node.
		headerHashes = append([]*BlockHash{bm._headerTip().Hash}, headerHashes...)
	}
...
func (bm *BitcoinManager) HandleBitcoinHeaders(conn net.Conn, msg *wire.MsgHeaders) error {
	glog.Debugf("BitcoinManager.HandleBitcoinHeaders: Processing %d headers "+
		"starting from height %d ending at height %d", len(msg.Headers),
	...
	if !isTimeCurrent {
		...
		// signal the Server to kick off its Ultranet sync. We do this now rather than
		...
		if hasBecomeTimeCurrent {
			glog.Info("BitcoinManager.HandleBitcoinHeaders: Bitcoin header chain has "+
			...
			// is actually invalid, we could end up in a state where some Ultranet blocks are
...
func ExtractBitcoinBurnTransactionsFromBitcoinBlock(
	bitcoinBlock *wire.MsgBlock, bitcoinBurnAddress string, params *UltranetParams) []*wire.MsgTx {
...
func ExtractBitcoinBurnTransactionsFromBitcoinBlockWithMerkleProofs(
	bitcoinBlock *wire.MsgBlock, burnAddress string, params *UltranetParams) (
...
func ExtractBitcoinExchangeTransactionsFromBitcoinBlock(
	bitcoinBlock *wire.MsgBlock, burnAddress string, params *UltranetParams) (
	_txns []*MsgUltranetTxn, _err error) {
	...
	for ii := range bitcoinBurnTxns {
		txHash := (BlockHash)(bitcoinBurnTxns[ii].TxHash())
		...
		// recipient of the Ultra that will be created is the first valid input in
		// the BitcoinTransaction specified. Note also that the
		// fee is deducted as a percentage of the eventual Ultra that will get
		// created as a result of this transaction.
		currentTxn := &MsgUltranetTxn{
...
func (bm *BitcoinManager) ProcessBitcoinBlock(bitcoinBlock *wire.MsgBlock) {
	...
	glog.Tracef("BitcoinManager.ProcessBitcoinBlock: Processing Bitcoin block "+
		"with (Hash: %v, Tstamp: %v, Num Txns: %d)", &blockHash,
		bitcoinBlock.Header.Timestamp, len(bitcoinBlock.Transactions))
	...
	glog.Tracef("BitcoinManager.ProcessBitcoinBlock: Found %d BitcoinExchange "+
		"transactions in block %v; may be invalid; forwarding to Server for processing",
		len(bitcoinExchangeTxns), (chainhash.Hash)(blockHash))
...
func (bm *BitcoinManager) _loop(conn net.Conn, params *UltranetParams, addrMgr *addrmgr.AddrManager, peerNetAddr *wire.NetAddress) {
	...
	for {
		...
		case msgWithError := <-readMsgChan:
			...
			if msgWithError.err != nil {
				unhandledCommand = strings.Contains(msgWithError.err.Error(), wire.WireErrorUnhandledCommand)
				if !unhandledCommand {
					glog.Errorf("BitcoinManager.readMsg: Encountered error reading message from "+
						"Peer. Finding new Peer and trying again %v: %v", conn.RemoteAddr().String(), msgWithError.err)
					return
				}
				continue
			}
			glog.Debugf(msgWithError.msg.Command())
			...
			case "addr":
				...
				addrMgr.AddAddresses(msg.AddrList, peerNetAddr)
			case "headers":
		...
		case <-time.After(time.Second):
			...
			if expectedRes != nil && expectedRes.TimeExpected.Before(time.Now()) {
				timeItTookToRespond := time.Now().Sub(expectedRes.TimeExpected).Seconds() + BitcoinGetHeadersTimeout.Seconds()
		...
		case switchPeerMsg := <-bm.switchPeerChan:
			...
			// be eventually fixed.
			newAddrAsNetAddr, err := IPToNetAddr(newConn.RemoteAddr().String(), bm.params)
			if err != nil {
...
func (bm *BitcoinManager) startSync() {
	...
	disconnectCount := 0
	for {
		conn, ipNetAddr := _connectToPeer(bm.addrMgr, bm.params.DialTimeout)
		if conn == nil {
			glog.Debugf("BitcoinManager.startSync: Trying a new Peer after a small break...")
		...
		bm._loop(conn, bm.params, bm.addrMgr, ipNetAddr)
